...
Bug fixes v3
------------
Elision (ἀλλ' ≈ ἀλλά) — two-part fix:
  1. Tokenisers now use _WORD_PUNCT_NONAPOS for stripping, so apostrophes
     survive into normalize_word instead of being stripped beforehand.
  2. normalize_word strips the trailing short vowel (α/ε/ι/ο) from FULL
     (non-elided) forms so both the elided stem and the full form converge
     on the same key (e.g. ἀλλ' and ἀλλά both → αλλ).
     The strip is NOT applied to elided tokens (ones that had an apostrophe)
     since they have already lost their final vowel via elision.

Nu-movable (ἐστίν ≈ ἐστί) — accent-insensitive penultimate check:
  The preceding-character check now strips accents from just that character
  before comparing, so it correctly identifies ί (U+03AF) as ι even when
  global accent-stripping is disabled.
"""
...
# Apostrophe / elision-marker codepoints found in Greek digital texts.
# FIX: expanded to include U+1FBD (GREEK KORONIS) and U+02BB (TURNED COMMA),
# both of which appear in TLG-style texts.
_APOSTROPHES = {
    "\u0027",  # APOSTROPHE                    '
...
_GREEK_SHORT_VOWELS = frozenset("αειο")

# ---------------------------------------------------------------------------
# Punctuation sets
# ---------------------------------------------------------------------------

# Clause-boundary characters (for word-repetition mode).
_CLAUSE_PUNCT = set(".,·;:\n")

# Characters to strip from word edges during tokenisation.
# FIX: apostrophe codepoints are EXCLUDED so they survive into normalize_word.
# (Previously they were in this set, stripping the elision mark before
#  normalize_word had a chance to detect and handle it.)
_WORD_PUNCT        = ".,·;:!?\"''\u2019\u02bc—–\u2013\u2014-()[]⟨⟩"
_WORD_PUNCT_NONAPOS = ".,·;:!?\"—–\u2013\u2014-()[]⟨⟩"
...
def normalize_word(word: str, opts: dict) -> str:
    ...
    Pipeline:
      1.  Elision check on RAW word — strip trailing apostrophe and record
          whether this token was elided.
      2.  NFD + lowercase.
      ...
      6.  NFC.
      7.  Nu-movable: strip final ν when the preceding BASE letter is ι or ε.
          The check is accent-insensitive (works even when strip_accents=False).
      8.  Elision stem: strip trailing short vowel (α/ε/ι/ο) from FULL forms
          (tokens that did NOT carry an apostrophe), so that ἀλλά normalises
          to the same key as the elided ἀλλ'.
    """
    w = word

    # ── Step 1: elision (must precede NFD so apostrophe codepoints survive) ─
    ...
    # stripping its combining marks before comparing.  This correctly handles
    # ί (U+03AF) as ι even when global strip_accents is False.
    if opts.get("handle_nu_movable", False):
        if len(w) >= 2 and w[-1] == "ν":
            prev_nfd  = unicodedata.normalize("NFD", w[-2])
            prev_base = unicodedata.normalize("NFC", "".join(
                c for c in prev_nfd if unicodedata.category(c) != "Mn"
            ))
            if prev_base in ("ι", "ε"):
                w = w[:-1]

    # ── Step 8: elision-stem normalisation ───────────────────────────────────
    # FIX: strip the trailing short vowel from FULL (non-apostrophe) forms so
    # that they converge on the same stem as the elided form.
    #   ἀλλ'  → had_apostrophe=True  → after step 1: αλλ  → step 8: no strip → αλλ
    #   ἀλλά  → had_apostrophe=False → after steps 2-6: αλλα → strip α     → αλλ  ✓
    #   δι'   → had_apostrophe=True  → after step 1: δι   → step 8: no strip → δι
    #   διά   → had_apostrophe=False → αλλ → ... → δια  → strip α           → δι   ✓
    if opts.get("handle_elision", False) and not had_apostrophe:
        if w and w[-1] in _GREEK_SHORT_VOWELS:
            w = w[:-1]
...
def tokenize_text(text: str) -> list[dict]:
    ...
    Tokenise `text` into a flat list of token dicts, each carrying:
        ...
        char_start         – char offset in `text` (for highlighting)
        clause_break_before – True if this token begins a new clause
        line_num           – 1-indexed line number

    FIX: uses _WORD_PUNCT_NONAPOS for stripping so apostrophes on elided
    words survive into normalize_word.
    """
    tokens = []
    pos = 0
    line_num = 1
    ...
    for raw_chunk in re.split(r'(\s+)', text):
        ...
        # FIX: strip without apostrophes
        word = raw_chunk.strip(_WORD_PUNCT_NONAPOS)
        if not word:
...
def tokenize_line(line: str, line_char_offset: int = 0) -> list[dict]:
    ...
    for raw in re.split(r'(\s+)', line):
        ...
        if re.match(r'^\s+$', raw):
            ...
            continue
        # FIX: strip without apostrophes
        word = raw.strip(_WORD_PUNCT_NONAPOS)
        if word:
...
def detect_repetitions(text: str):
    ...
    for occ in occurrences:
        ...
        for ui, phrase_toks in zip(occ["unit_indices"], occ["phrases"]):
            ld = line_data[ui]
            full_toks = ld["tokens"]
            phrase_ids = {id(t): t for t in phrase_toks}
            for ti, ft in enumerate(full_toks):
                if id(ft) in phrase_ids:
