...
/* ----------------------------
   DEMO TEXT
   Pre-loaded into the paste area on page load so a presenter can click
   "Run" immediately without typing anything — used for conference /
   workshop demos (e.g. Theocritus, Idyll 6).
----------------------------- */

const DEMO_TEXT = `Δαμοίτας χὡ Δάφνις ὁ βουκόλος εἰς ἕνα χῶρον
...
document.addEventListener("DOMContentLoaded", () => {
    // Pre-fill the paste area with the demo text (Theocritus, Idyll 6) so a
    // presenter can click "Run" immediately without typing or uploading.
    const pasteAreaEl = document.getElementById("pasteArea");
    pasteAreaEl.value = DEMO_TEXT;
    loadedText = DEMO_TEXT;
    setStatus(`Demo text loaded (Theocritus, Idyll 6 — ${DEMO_TEXT.length.toLocaleString()} chars). Click "Run Hiatus Detector" to see it in action, or replace it with your own text.`);
